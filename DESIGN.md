...
the plate prints mirrored. the photograph's tallest ridge sits at its left edge and the skyline runs downhill from there, which put the heaviest part of the frame under the flush-left hero copy and left the right of the page with the featureless near slope. mirrored, the copy gets the low end of the range and the peaks carry the side of the page that has nothing else on it. the desktop crop also sits up to 1.4x tighter than the full frame. the whole trim comes off the plate's right end because the tallest peak in the photograph is at its left one; the zoom is taken about the top of the ridge, so the skyline stays where it is and the range grows underneath it rather than sliding down the viewport; and it is capped per layout so the whole skyline stays inside the part of the band the viewport shows, which on an ultrawide leaves room for almost no zoom at all.
