...
the plate prints mirrored. the photograph's tallest ridge sits at its left edge and the skyline runs downhill from there, which put the heaviest part of the frame under the flush-left hero copy and left the right of the page with the featureless near slope. mirrored, the copy gets the low end of the range and the peaks carry the side of the page that has nothing else on it.

the desktop terrain band is 0.68 of the viewport rather than 0.55, and that is the only lever there is for how much of the page the range occupies. the plate is printed at full width, so its magnification is fixed at viewport width over plate width: making the mountain bigger means cropping the photograph, which costs peaks. a taller band costs nothing -- it shows more of the same plate and starts the ridge higher up the page. the mirror is what makes the taller band safe, because the end of the range that now sits under the hero copy is its low end.
