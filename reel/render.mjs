...
//
//   node reel/render.mjs            cues + frames + mux  -> reel/out/reel.mp4
//   node reel/render.mjs --cues     only write reel/out/cues.json
//
...
const to = Number(args.find((a) => a.startsWith("--to="))?.split("=")[1] ?? 30);
const scale = Number(args.find((a) => a.startsWith("--scale="))?.split("=")[1] ?? 1);

await mkdir(out, { recursive: true });
const server = await serve();
const url = `http://127.0.0.1:${server.address().port}/reel/index.html?capture`;
